...
export const CADENCE = {
  ...
  // Every visibility check spends real money on model calls and web searches,
  // so it stays weekly and opt-in per site rather than sweeping every site by
  // default.
  [JOB_TYPES.AI_VISIBILITY]: { everyMs: 7 * 24 * 60 * 60 * 1000, label: "weekly", optIn: true },
};
...
export function dueWorkFor(site, now = Date.now()) {
  ...
  for (const [type, { everyMs, optIn }] of Object.entries(CADENCE)) {
    // A competitor scan with nothing to scan is not work.
    if (type === JOB_TYPES.COMPETITOR_SCAN && !(site.competitorCount > 0)) continue;
    // Paid work only runs where the owner asked for it — and only once the dial
    // is at "Let it rip" or above. Below that the product promises to look and
    // report, and a weekly re-check that spends money is acting, not reporting.
    //
    // This is the one thing the autonomy setting gates server-side today.
    // Everything that publishes is held for a person by design at every level,
    // so there is nothing else for the dial to release yet.
    if (optIn && !site.autoVisibility) continue;
    if (optIn && (site.autonomy ?? 62) < 48) continue;
    // Visibility questions are derived from the crawl; without it they'd be
    // generic guesses not worth paying for.
    if (type === JOB_TYPES.AI_VISIBILITY && !site.intelligence) continue;
