...
//    than implying tracking that isn't happening. Adding AdSense meant adding
//    it here in the same commit, and correcting three places on the site that
//    said this product carried no advertising.
...
// Bumped when the inventory changes, which invalidates every stored answer and
// re-asks. Adding AdSense is exactly that case: an existing "reject optional"
// was an answer to a question that did not mention advertising.
...
export const INVENTORY = {
  ...
  [CATEGORIES.MARKETING]: {
    label: "Advertising",
    description:
      "Google AdSense serves the ads on this site. Allowing it lets Google read and write its own cookies here and use them to choose which ads you see, including on other sites. Refusing does not remove the ads, it removes the profiling behind them.",
    required: false,
    items: [
      {
        key: "__gads / __gpi / IDE",
        kind: "Third-party cookies",
        purpose: "Chooses which ads to show you and measures whether you interacted with them.",
        retention: "Up to 13 months",
        party: "Google AdSense",
      },
      {
        key: "adsbygoogle.js",
        kind: "Third-party script",
        purpose:
          "Loads the ad slots. Only loaded once this category is allowed, so refusing means the script is never fetched at all rather than fetched and told to behave.",
...
function defaults(model) {
  ...
  return {
    ...
    // Advertising follows the same rule as the rest, now that there is
    // actually an ad network here. Under opt-out law targeted advertising is
    // lawful until the visitor says no, and they can say no from the footer at
    // any time. Under opt-in it waits for a yes, and `allows()` refuses it
    // outright whenever Global Privacy Control is set, whatever is stored.
    //
    // It used to be hardcoded off everywhere, which was correct when the
    // category was empty and would now just mean the ads never load.
    [CATEGORIES.MARKETING]: on,
  };
}
