...
export async function reserve({
  ...
  // Counted per post rather than per drafting run, because a run that drafts for
  // four networks produces four things that can go out. Charging one would let a
  // Growth customer publish 160 posts against an allowance of 40.
  if (socialPosts && usage.socialPosts + socialPosts > (plan.socialPosts || 0)) {
    return {
      ok: false,
      code: "social_exhausted",
      reason: plan.socialPosts
        ? `You've used this month's ${plan.socialPosts.toLocaleString()} social posts on ${plan.name}.`
        : `${plan.name} doesn't include social posting.`,
      used: usage.socialPosts,
      allowance: plan.socialPosts || 0,
    };
  }
    ...
    usageRef(db, uid, siteId, monthKey()).set(
      {
        ...
        ...(socialPosts ? { socialPosts: FieldValue.increment(socialPosts) } : {}),
  ...
  return { ok: true, hold: { uid, siteId, action, job, estimate, credits: cost, leadCredits, emails, contentPieces, socialPosts } };
}
...
export async function record(hold, { inputTokens = 0, outputTokens = 0, webSearches = 0, failed = false } = {}) {
  ...
  const { uid, siteId, job, estimate, credits, leadCredits, emails, contentPieces, socialPosts } = hold;
  const db = adminDb();
  ...
  if (failed) {
    ...
    if (socialPosts) patch.socialPosts = FieldValue.increment(-socialPosts);
  }
 ...
 * call throws.
 */
export async function metered({ uid, siteId, subscription, action, job, leadCredits, emails, contentPieces, socialPosts }, run) {
  const gate = await reserve({ uid, siteId, subscription, action, job, leadCredits, emails, contentPieces, socialPosts });
