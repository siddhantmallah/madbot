...
export async function reserve({ uid, siteId, subscription, action, job, leadCredits = 0, emails = 0, contentPieces = 0 }) {
  ...
  return { ok: true, hold: { uid, siteId, action, job, estimate, credits: cost } };
}
...
export async function record(hold, { inputTokens = 0, outputTokens = 0, webSearches = 0, failed = false } = {}) {
  ...
  const { uid, siteId, job, estimate, credits } = hold;
  const db = adminDb();
  ...
  // A failed call still cost whatever it burned before failing, but the customer
  // shouldn't pay credits for work they didn't get.
  if (failed) patch.credits = FieldValue.increment(-credits);
