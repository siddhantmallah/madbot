...
async function commitWrites(db, uid, siteId, writes) {
  ...
  if (Array.isArray(writes.leads)) {
    writes.leads.slice(0, 400).forEach((l) => {
      batch.set(
        db.collection("users").doc(uid).collection("sites").doc(siteId).collection("leads").doc(l.id),
...
export async function runJobAdmin(db, uid, siteId, jobId) {
  ...
  if (outcome.status === JOB_STATUS.COMPLETED) {
    ...
    // Usage travels back so the caller can replace its cost estimate with what
    // the work actually cost. Same three places the interactive route looks.
    return {
      completed: true,
      summary: outcome.summary,
      usage: outcome.usage || outcome.result?.usage || outcome.writes?.site?.aiVisibility?.usage || null,
    };
  }
