...
export function digestData({ site, activity, approvals, leads, content, competitors = [], search = null }) {
  ...
  // MADBOT never sends outreach itself, so nothing is ever "sent". Report the
  // stage leads actually reach.
  const qualifiedLeads = leads.filter((l) => l.stage === "qualified");
  const published = content.filter((c) => c.status === "published");
  // Only pieces that actually have an article. This goes out by email, so a
  // planned topic counted as a draft is a wrong number in a customer's inbox.
  const drafts = content.filter((c) => c.article && c.status !== "published");
  ...
  const lines = [
    ...
    { label: "Prospects", value: `${leads.length} found, ${qualifiedLeads.length} qualified` },
  ];
