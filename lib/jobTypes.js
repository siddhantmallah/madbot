...
// Social drafting and directory copy are deliberately NOT job types. A job is
// something the scheduler can start on its own; both of those need a person to
// say what the material is, and they finish inside one request. They run through
// /api/social/draft and /api/listings/prepare, the same way article writing runs
// through /api/generate-content rather than the queue.
 ...
 * job it maps to for cost estimation. A job absent from here spends nothing.
 *
 * This lives here rather than in a route because BOTH run paths need it. It
 * used to be a private constant inside /api/jobs/run, which is exactly why the
 * scheduled path metered nothing: the cron could not see it.
 */
export const JOB_METER = {
