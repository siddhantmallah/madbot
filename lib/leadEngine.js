...
import { observeCompany, scoreOpportunity, signalsForProduct } from "./leadSignals";
import { officialProfile } from "./officialSources";
...
export async function buildBuyerProfile({ intel }) {
  ...
  const res = await client.messages.parse({
    ...
    max_tokens: 3000,
    output_config: { format: { type: "json_schema", schema: ICP_SCHEMA } },
    system: [
...
export async function shortlist({ companies, profile, max = 12, onProgress }) {
  ...
  // Which technical signals matter for what this customer sells, so a
  // monitoring product and a security product are not scored the same way.
  const productSignals = signalsForProduct(profile);
  ...
  for (const c of companies) {
    ...
    // The point of doing this before any model runs: certificates, DNS and
    // headers are free, deterministic and not personal data, so every
    // candidate can be scored properly before anything expensive sees a
    // shortlist. It is what makes "expires in nine days, nothing watching
    // it" a ranking signal rather than a nice-to-have.
    const observation = await observeCompany(c.domain);
    const score = scoreOpportunity({ observation, productSignals });
    ...
    if (onProgress) onProgress({ shortlisted: kept.length, checked: kept.length + rejected.length });
  }

  // Rank by the deterministic score, then cap. The previous version filled
  // the shortlist in arrival order, so a company with an expired certificate
  // could be dropped for being checked late.
  kept.sort((a, b) => b.score.overall - a.score.overall);
  const shortlisted = kept.slice(0, max);
  kept.slice(max).forEach((c) =>
    rejected.push({ ...c, reason: `scored ${c.score.overall}, below the top ${max}` })
  );
...
export async function qualifyCompany({ company, profile, customerDomain }) {
  ...
  // Official registries, at the point where the extra latency is justified —
  // only companies that survived the shortlist get looked up.
  const official = await officialProfile({ domain: company.domain, companyName: company.name });
  ...
  const res = await client.messages.parse({
    ...
    system: [
      ...
      "",
      "Judge from their own site and the measured infrastructure facts supplied. Do not speculate about funding, revenue or headcount — anything not given to you, you do not know. If evidence is thin, score low and say so: a confident wrong answer costs the customer a wasted approach and their reputation.",
      "The infrastructure facts were measured directly. Treat them as true, cite them where relevant, and never contradict them or invent similar-sounding ones.",
    ...
    messages: [
      {
        ...
        content: [
          `Buyer profile:`,
          JSON.stringify({ sells: profile.sells, buyerType: profile.buyerType, sectors: profile.sectors, triggers: profile.triggers }, null, 1),
          ``,
          `Company: ${company.name} (${company.domain})`,
          ``,
          // Measured facts, handed over rather than left for the model to
          // guess at. It should reason about these, not invent equivalents.
          `Observed infrastructure (measured directly, not inferred):`,
          observationSummary(company.observation),
          ``,
          `Official records:`,
  ...
  return {
    ...
    ...q,
    official,
    registrantCountry: official.registrantCountry || null,
...
// ---------------------------------------------------------------------------
// Stage 5 — the outreach draft, for a person to send
// ---------------------------------------------------------------------------

const OUTREACH_SCHEMA = {
  type: "object",
  properties: {
    subject: { type: "string", description: "Under 60 characters. Specific to this company. No clickbait." },
    body: {
      type: "string",
      description: "The full email as plain text, ready to send. Short paragraphs. Ends with the placeholder [Your name] on its own line.",
    },
    confidence: {
      type: "number",
      description: "0 to 1: how sure you are that every factual claim about the recipient is supported by what was observed.",
    },
    claims: {
      type: "array",
      description: "Every factual claim the email makes about the recipient company, one per item.",
      items: { type: "string" },
    },
  },
  required: ["subject", "body", "confidence", "claims"],
  additionalProperties: false,
};

/**
 * Drafts the outreach email. Drafts — a person sends it, from their own
 * address, at every autonomy level. Until then it sits in the approvals queue.
 *
 * Everything the email says about the recipient must come from what was
 * actually observed during qualification. A cold email that opens with an
 * invented compliment is the fastest route to a spam complaint, and the
 * complaint lands on the customer's domain, not ours.
 */
export async function draftOutreachEmail({ lead, profile, siteContext, voice, rules }) {
  requireKey();
  const client = new Anthropic();
  const guardrails = (rules || []).map((r) => `- ${r.text || r}`).join("\n");

  const res = await client.messages.parse({
    model: modelFor("lead_outreach").id,
    max_tokens: 1500,
    ...thinkingFor("lead_outreach"),
    output_config: { effort: "low", format: { type: "json_schema", schema: OUTREACH_SCHEMA } },
    system: [
      "You draft one short business-to-business email from a company to a prospect.",
      "",
      "Refer only to things that were actually observed about the prospect and are listed below. Invent nothing — no assumed pain, no flattery about work you have not seen.",
      "Open with the specific observation, not with who the sender is. Two or three short paragraphs. One clear, low-pressure ask.",
      "Name the sender's business, and include a plain line offering to stop — 'if this isn't relevant, say so and I won't write again'. It is required in most jurisdictions, and it is decent.",
      "No subject-line tricks. Not 'quick question'. Not 'I hope this finds you well'.",
      "End the body with the literal placeholder [Your name] on its own line.",
      guardrails ? `\nThe sender's standing rules, which override everything above:\n${guardrails}` : null,
      voice ? `\nVoice: ${voice}` : null,
    ]
      .filter(Boolean)
      .join("\n"),
    messages: [
      {
        role: "user",
        content: [
          `Sender: ${siteContext?.name || "the business"} (${siteContext?.domain || ""})`,
          `What the sender sells: ${profile?.sells || siteContext?.summary || "(unknown)"}`,
          "",
          `Prospect: ${lead.co || lead.domain} (${lead.domain})`,
          lead.problemYouSolve ? `The problem they appear to have: ${lead.problemYouSolve}` : null,
          lead.why ? `Why they qualified: ${lead.why}` : null,
          lead.openingLine ? `A suggested opening that references something real on their site: ${lead.openingLine}` : null,
          lead.intentSignals?.length ? `Observed signals: ${lead.intentSignals.join("; ")}` : null,
          lead.contactIsGeneric ? "The address is a shared company inbox — do not address a named person." : null,
        ]
          .filter((l) => l !== null)
          .join("\n"),
      },
  ...
  if (res.stop_reason === "refusal") {
    throw Object.assign(new Error("Declined to draft outreach to this company."), { code: "refused" });
  }
  const out = res.parsed_output;
  if (!out?.body) throw Object.assign(new Error("No draft returned."), { code: "empty" });
  ...
  return {
    ...
    claims: out.claims || [],
    usage: { inputTokens: res.usage?.input_tokens || 0, outputTokens: res.usage?.output_tokens || 0, webSearches: 0 },
  };
}

/** Measured infrastructure, phrased so a model can reason about it. */
function observationSummary(obs) {
  if (!obs) return "  (not observed)";
  const out = [];
  const { ssl, dns, headers, tech, statusPage } = obs;
  if (ssl?.ok) {
    out.push(
      `  TLS: issued by ${ssl.issuer}, ${ssl.lifetimeDays}-day lifetime, ` +
        `${ssl.expired ? "ALREADY EXPIRED" : `expires in ${ssl.daysToExpiry} days`}, ` +
        `${ssl.sanCount} hostname(s), renewal looks ${ssl.likelyAutomated ? "automated" : "manual"}`
    );
  } else {
    out.push(`  TLS: could not be read (${ssl?.reason || "unknown"})`);
  }
  if (dns?.ok) {
    out.push(
      `  DNS: mail via ${dns.mailProvider || "none"}, SPF ${dns.hasSpf ? "yes" : "no"}, ` +
        `DMARC ${dns.hasDmarc ? "yes" : "no"}, nameserver ${dns.nameserver || "unknown"}`
    );
  }
  if (headers?.ok) {
    out.push(`  Headers: ${headers.securityHeaderCount} of 5 basic security headers, server ${headers.server || "unstated"}`);
  }
  if (tech?.ok) {
    out.push(
      `  Stack: ${tech.stack || "unrecognised"}, analytics ${tech.analytics ? "present" : "absent"}, ` +
        `error tracking ${tech.monitoring?.errorTracking ? "present" : "not seen on the homepage"}`
    );
  }
  out.push(`  Status page: ${statusPage?.found ? `yes, at ${statusPage.host}` : "none found at status./uptime./health."}`);
  return out.join(String.fromCharCode(10));
}

/** Registry facts, with absences stated rather than quietly omitted. */
function officialSummary(official) {
  if (!official) return "  (not looked up)";
  const src = official.sources || {};
  const out = [];
  if (src.rdap?.ok) {
    out.push(`  Domain registered ${String(src.rdap.registeredAt || "").slice(0, 10)} (${src.rdap.ageYears} years old), registrar ${src.rdap.registrar || "unknown"}`);
  }
  if (src.certificateTransparency?.ok) {
    out.push(
      `  Certificate Transparency: ${src.certificateTransparency.certificateCount} certificates, ` +
        `${src.certificateTransparency.distinctHostnames} distinct hostnames, ` +
        `${src.certificateTransparency.issuedLast30Days} issued in the last 30 days`
    );
  } else {
    // Said explicitly, so an unavailable source is not read as a small estate.
    out.push("  Certificate Transparency: unavailable, so estate size is unknown rather than small");
  }
  if (src.sec?.ok && src.sec.found) out.push(`  SEC filer: ${src.sec.registeredName} (${src.sec.ticker}) — a public company`);
  if (src.companiesHouse?.ok && src.companiesHouse.found) {
    out.push(`  Companies House: ${src.companiesHouse.registeredName}, ${src.companiesHouse.status}, incorporated ${src.companiesHouse.incorporatedAt}`);
  }
  if (official.notConfigured?.length) out.push(`  Not consulted (no API key): ${official.notConfigured.join(", ")}`);
  return out.length ? out.join(String.fromCharCode(10)) : "  (nothing found)";
}
