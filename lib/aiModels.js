...
export const MODELS = {
  cheap: {
    ...
    inputPerMTok: 1.0,
    outputPerMTok: 5.0,
    // Not a guess. The live API answers this model with
    // 400 invalid_request_error "adaptive thinking is not supported on this
    // model", which took out every cheap-tier call: the visibility question
    // planner, competitor extraction, the fact-checker, lead classification
    // and the social angle pass. Every one of them sent it unconditionally.
...
export const JOBS = {
  ...
  // Finding the angles is comprehension over material we already hold, so it
  // runs cheap. The draft itself is the one that goes out under the customer's
  // name, so it doesn't.
  social_angles: { tier: "cheap", label: "Find angles worth posting" },
  social_draft: { tier: "standard", label: "Write a social post" },
  // Deliberately not the same model that wrote the post. A model checking its
  // own output for overclaiming grades itself generously; a different one, told
  // only to find breaches, does not.
  social_guardrail: { tier: "cheap", label: "Check a post against the rules" },
  listing_copy: { tier: "standard", label: "Write a directory listing" },
};
 ...
 */
/**
 * The `thinking` parameter for a job, or nothing at all.
 *
 * Spread this into a messages call rather than writing `thinking` by hand:
 *
 *   const res = await client.messages.parse({
 *     model: SOME_MODEL,
 *     ...thinkingFor("visibility_questions"),
 *     ...
 *   });
 *
 * Ten call sites hardcoded `thinking: { type: "adaptive" }` regardless of which
 * model the job resolved to, so every cheap-tier call failed with a 400 the
 * moment a working API key was in place. The capability belongs to the model,
 * so the decision belongs here, next to the model table.
 */
export function thinkingFor(job) {
  return modelFor(job).adaptiveThinking ? { thinking: { type: "adaptive" } } : {};
}
