...
import { buildProvenance, policyCheck, jurisdictionFor, LEVELS, DECISIONS } from "./dataPolicy";
    ...
    writes: {
      ...
      leads: shortlisted.map((c) => ({
        ...
        data: {
          ...
          domain: c.domain,
          stage: "shortlisted",
          // Company-level only at this stage: no personal data, so no lawful
          // basis needed — and the record says so rather than leaving it blank.
          provenance: buildProvenance({
            level: LEVELS.COMPANY,
            domain: c.domain,
            source: "Public web search and the company's own homepage",
...
HANDLERS[JOB_TYPES.LEAD_QUALIFY] = async function leadQualify(job, { step }) {
  ...
  for (const [i, company] of companies.entries()) {
    ...
    // Contact discovery is where the data level changes from company to
    // person, so the policy is consulted before it happens rather than after.
    let contact = null;
    let contactPolicy = null;
    if (q.qualified && q.personLookupJustified) {
      const jurisdiction = jurisdictionFor({ domain: company.domain });
      // Generic inboxes and named individuals are different levels, but we
      // don't know which we'll find until we look — so check against the
      // stricter of the two first.
      contactPolicy = policyCheck({ level: LEVELS.NAMED_PERSON, jurisdiction });
      if (contactPolicy.decision === DECISIONS.NEVER) {
        contact = { skipped: contactPolicy.reason, policy: contactPolicy };
      } else {
        step("find a contact route", company.domain);
        contact = await findContactRoute({ company: q });
        contact.policy = contactPolicy;
      }
    }
    out.push({ ...q, contact, contactPolicy });
  }
  ...
  return {
    ...
    writes: {
      leads: out.map((q) => ({
        ...
        data: {
          stage: q.skipped ? "not-assessed" : q.qualified ? "qualified" : "rejected",
          // A refusal is not a score of zero. leadEngine returns skipped:true
          // when the model declines, and this used to drop that flag, so the
          // screen mapped 0 to "Cold" and a company nobody assessed looked
          // like one that had been assessed and found cold.
          skipped: !!q.skipped,
          fit: q.skipped ? null : q.score >= 70 ? "Hot" : q.score >= 40 ? "Warm" : "Cold",
          score: q.skipped ? null : q.score ?? 0,
          ...
          // The full record, so "why do we have this?" has an answer that was
          // written at collection time rather than reconstructed later.
          provenance: q.contact?.preferred
            ? buildProvenance({
                level: q.contact.isGenericInbox ? LEVELS.ROLE_INBOX : LEVELS.NAMED_PERSON,
                domain: q.domain,
                source: "Published on the company's own page",
                sourceUrl: q.contact.source || null,
                status: q.qualified ? "active" : "rejected",
              })
            : buildProvenance({
                level: LEVELS.COMPANY,
                domain: q.domain,
                source: "The company's own pages",
                sourceUrl: `https://${q.domain}`,
                status: q.qualified ? "active" : "rejected",
              }),
          contactPolicy: q.contactPolicy || null,
          contactSkipped: q.contact?.skipped || null,
