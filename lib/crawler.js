...
/**
 * Does robots.txt exclude this path?
 *
 * Handles the two wildcards the major crawlers support: `*` for any run of
 * characters, and a trailing `$` to anchor the end. This was a plain
 * startsWith, so a rule like `Disallow: /*?` or `Disallow: /*.pdf$` was
 * counted in disallowCount and could never match anything. The crawler fetched
 * paths the site had excluded while reporting that it had read the rule.
 *
 * Matched with string operations rather than a built regex, so a pattern out
 * of somebody else's robots.txt can never be a regex injection or a
...
function isDisallowed(pathname, rules) {
  return rules.disallow.some((pattern) => {
    ...
    // The text before the first wildcard has to be a prefix.
    ...
    for (let k = 1; k < parts.length; k += 1) {
      ...
      if (anchored && k === parts.length - 1) {
        return pathname.length >= at + part.length && pathname.endsWith(part);
      }
      const found = pathname.indexOf(part, at);
      if (found === -1) return false;
      at = found + part.length;
    }

    // An anchored pattern ending in a wildcard matches anything from here.
    return true;
  });
}
...
export async function crawlSite(input, { maxPages = 20, maxDepth = 2, budgetMs = 120_000, onProgress } = {}) {
  ...
  const pages = [];
  // Every URL any crawled page links to, collected as we go so the orphan
  // check at the end has something to compare against.
  ...
  while (queue.length && pages.length < maxPages) {
    ...
    results.forEach(({ url, depth, source, res }) => {
      ...
      // Remember what this page links to BEFORE dropping the array. The
      // orphan pass below used to read `outLinks` after this delete had
      // already run, so its set was always empty and every page below the
      // homepage was reported as an orphan, including ones linked straight
      // from it. Every "orphan page" the product has ever shown was wrong.
      page.outLinks.forEach((l) => {
        linkedTo.add(l);
        push(l, depth + 1, "link");
      });
  ...
  // `linkedTo` is filled during the crawl, above.
