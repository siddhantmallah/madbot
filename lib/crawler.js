...
export async function crawlSite(input, { maxPages = 20, maxDepth = 2, budgetMs = 120_000, onProgress } = {}) {
  ...
  const push = (url, depth) => {
    ...
    seen.add(c);
    queue.push({ url: c, depth });
  };

  push(start.toString(), 0);
  SEED_PATHS.forEach((p) => push(`${origin}${p}`, 1));
  sitemapUrls.slice(0, maxPages * 2).forEach((u) => push(u, 1));
  ...
  while (queue.length && pages.length < maxPages) {
    ...
    results.forEach(({ url, depth, res }) => {
      if (!res || res.error || !res.ok) {
        errors.push({ url, status: res?.status ?? null });
        return;
      }
      ...
      page.outLinks.forEach((l) => push(l, depth + 1));
