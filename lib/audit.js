...
import { THRESHOLDS as T } from "./auditClient";
...
export async function runSnapshot(input) {
  ...
  const navPaths = [...html.matchAll(/<a[^>]+href=["']([^"']+)["']/gi)]
  ...
  return {
    ...
    paths: [...new Set(navPaths)].filter((p) => p !== "/").slice(0, 60),
...
/**
 * Does this robots.txt actually shut every crawler out?
 *
 * The old test was a single regex with a lazy wildcard between the user-agent
 * and the disallow, which walked straight out of the star group and into any
 * later one. So this entirely ordinary file:
 *
 *     User-agent: *
 *     Disallow: /admin
 *
 ...
 *     Disallow: /
 *
 * earned a critical "robots.txt blocks all crawlers", a twenty point penalty
 * and the line "Nothing else matters until this is fixed". It also disagreed
 * with lib/crawler.js, which parses the same file into groups correctly, so
 * the audit and the crawl contradicted each other about the same site.
 *
 * Group-scoped, the way the standard reads: only a Disallow of "/" inside a
 * group that applies to everyone counts.
 */
function robotsBlocksEveryone(body) {
  ...
  for (const raw of lines) {
    ...
    if (key === "user-agent") {
      // Consecutive user-agent lines form a single group, so the flag only
      // resets when a new block starts after a directive.
      if (!sawAgentLine) inStarGroup = false;
      sawAgentLine = true;
    ...
    sawAgentLine = false;
    if (inStarGroup && key === "disallow" && value === "/") return true;
  }
  return false;
}
...
export async function runAudit(input) {
  ...
  const robots = {
    ...
    blocksAll: robotsBlocksEveryone(robotsBody),
  };
