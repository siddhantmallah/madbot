...
export function addCompetitor(uid, siteId, { url, snapshot }) {
  return addDoc(competitorsCol(uid, siteId), {
    ...
    snapshot,
    changes: [],
    // Deliberately NOT set here. One snapshot is not a comparison, and the
    // panel keys "No changes since the last check" off this field, so stamping
    // it at creation reported the outcome of a check that never happened.
    // Written for the first time by the first real check.
    lastCheckedAt: null,
    createdAt: serverTimestamp(),
  });
}
