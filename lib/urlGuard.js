  ...
  try {
    ...
    for (let hop = 0; ; hop += 1) {
      ...
      if (binary) {
        ...
        return { ok: res.ok, status: res.status, finalUrl: current.toString(), bytes: received, contentType, data: out, elapsedMs };
      }
      ...
      if (reader) {
        ...
        reader.cancel().catch(() => {});
        return { ok: res.ok, status: res.status, finalUrl: current.toString(), body, bytes: received, contentType, elapsedMs };
      }

      body = (await res.text()).slice(0, capBytes);
      return { ok: res.ok, status: res.status, finalUrl: current.toString(), body, bytes: body.length, contentType, elapsedMs };
    }
  } finally {
    clearTimeout(timer);
  }
