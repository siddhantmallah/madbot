 ...
 */
export async function safeFetch(url, { timeoutMs = 8000, capBytes = 400_000, maxRedirects = MAX_REDIRECTS, binary = false } = {}) {
  ...
  try {
    ...
    for (let hop = 0; ; hop += 1) {
      ...
      const res = await fetch(current, {
        ...
        headers: {
          "User-Agent": "MADBOTBot/1.0 (+https://getmadbot.com)",
          // Stated explicitly because the audit reports on whether the server
          // compressed its reply. Asking for it is the only way that finding
          // means anything — a server that was never offered gzip and did not
          // use it has done nothing wrong.
      ...
      // The audit reads security, caching and compression headers off this.
      // undici decodes the body but leaves `content-encoding` in place, so it
      // still says whether the bytes arrived compressed.
      const headers = res.headers;

      // Images and other non-text resources go through the same guarded loop
      // rather than a second fetch somewhere else that somebody would
      // eventually forget to validate.
      if (binary) {
        const chunks = [];
        let received = 0;
        ...
        for (const c of chunks) {
          out.set(c.subarray(0, Math.min(c.length, received - at)), at);
          at += c.length;
          if (at >= received) break;
        }
        return { ok: res.ok, status: res.status, finalUrl: current.toString(), bytes: received, contentType, headers, data: out, truncated: received >= capBytes, elapsedMs };
      }
      ...
      if (reader) {
        ...
        reader.cancel().catch(() => {});
        // Callers need to know they are reading a partial document. A report
        // that says "no privacy policy" because the footer fell off the end of
        // the cap is worse than one that says it could not see the whole page.
        return { ok: res.ok, status: res.status, finalUrl: current.toString(), body, bytes: received, contentType, headers, truncated: received >= capBytes, elapsedMs };
      }

      body = (await res.text()).slice(0, capBytes);
      return { ok: res.ok, status: res.status, finalUrl: current.toString(), body, bytes: body.length, contentType, headers, truncated: body.length >= capBytes, elapsedMs };
    }
  } finally {
    clearTimeout(timer);
  }
