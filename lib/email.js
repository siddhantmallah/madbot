...
/**
 * The confirm-your-address mail.
 *
 * Sent through Resend on the verified domain rather than by Firebase, because
 * this is the very first thing a new customer receives and it should not
 * arrive from a Google address with a Google template. The link itself is
 * generated by the Admin SDK, so it is a real Firebase action link with a real
 * expiry - nothing here weakens the verification.
 *
 * Deliberately short. A verification mail that has to be read is a worse
 * verification mail.
 */
export function buildVerifyEmail({ name, link, contactEmail = CONTACT_EMAIL }) {
  const first = (name || "").trim().split(/\s+/)[0];
  const greeting = first ? `Hi ${first},` : "Hi,";

  const subject = "Confirm your email address for MADBOT";

  const body = `
<tr><td style="padding-bottom:18px;">
  <span style="font-size:21px;font-weight:700;color:#fff;">Confirm your email address</span>
</td></tr>
<tr><td style="padding-bottom:16px;">${greeting}</td></tr>
<tr><td style="padding-bottom:16px;">
  One click and your account is ready. This also starts your free trial, which is why we ask before
  switching anything on.
</td></tr>
<tr><td style="padding-bottom:22px;">
  <a href="${link}" style="display:inline-block;background:#FF6B35;color:#0A0810;text-decoration:none;font-weight:700;padding:12px 22px;border-radius:999px;">Confirm my email address</a>
</td></tr>
<tr><td style="padding-bottom:16px;color:rgba(237,234,245,.62);font-size:13px;word-break:break-all;">
  If the button does not work, paste this into your browser:<br>
  <a href="${link}" style="color:#FF9068;">${link}</a>
</td></tr>
<tr><td style="padding-bottom:6px;color:rgba(237,234,245,.62);font-size:13.5px;">
  If you did not create a MADBOT account, ignore this email and nothing will happen. The link expires on its own.
  ${contactEmail ? `Questions: <a href="mailto:${contactEmail}" style="color:#FF9068;">${contactEmail}</a>.` : ""}
</td></tr>`;

  const text = `${greeting}

Confirm your email address to finish setting up your MADBOT account. This also starts your free trial.
