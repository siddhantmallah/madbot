...
export function validatePost({ networkId, text, images = [], premium = false }) {
  ...
  if (!body && !(net.imageRequired && images.length)) {
  ...
  if (length > limit) {
    ...
    problems.push(
      net.linkCharCost && /https?:\/\//.test(body)
        ? `${over} characters over ${net.label}'s ${limit} limit — every link counts as ${net.linkCharCost} there, whatever its length.`
        : `${over} characters over ${net.label}'s ${limit} limit.`
    );
  }
