...
export default function ActivityLog({ feedAll, undone, onToggleUndo }) {
  const [filt, setFilt] = useState("all");
  const rows = feedAll.filter((f) => filt === "all" || undone[f.id]);
    ...
    <section data-screen-label="Activity log" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", alignItems: "flex-end", gap: 14, flexWrap: "wrap" }}>
        <div>
          ...
          <p className="text-muted" style={{ margin: 0, fontSize: 13.5 }}>1,406 actions, each one reversible. Nothing happens off the record.</p>
        </div>
            ...
            {rows.map((f) => {
              ...
              return (
                <tr key={f.id} style={{ opacity: u ? 0.55 : 1 }}>
                  <td style={{ whiteSpace: "nowrap", fontSize: 12 }} className="text-muted">{ago(f.m)}</td>
                  ...
                  <td style={{ textAlign: "right" }}>
                    <button className="btn btn-ghost" onClick={() => onToggleUndo(f.id)} style={{ fontSize: 12, fontWeight: 600 }}>
                      {u ? "Restore" : "Roll back"}
                    </button>
                  </td>
                </tr>
